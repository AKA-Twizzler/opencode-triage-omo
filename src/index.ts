/*
 * opencode-triage-omo — Agent + Skill Router Plugin
 * ===================================================
 * Version: 1.3.3
 * License: MIT
 *
 * Extended OMO triage: routes to agents, skills, and tools via keyword scoring
 * for OpenCode. Forked from cascharly/opencode-triage.
 *
 * Registers a `triage()` custom tool that discovers SKILL.md files and OMO
 * agent definitions, then routes LLM queries to matching skills/agents via
 * keyword scoring.
 *
 ...
 *
 * Install:  { "plugin": ["opencode-triage-omo"] }  in opencode.json
...
async function checkForUpdate(tui: any): Promise<void> {
  try {
    ...
    const res = await fetch("https://registry.npmjs.org/opencode-triage-omo/latest", {
    ...
    if (latest && semverGt(latest, CURRENT_VERSION)) {
      await tui.showToast({
        body: {
          message: `Update available: ${CURRENT_VERSION} → ${latest} — npm install -g opencode-triage-omo@latest`,
          variant: "warning",
        },
      })
    }
...
/**
 * Triage agent + skill router plugin — main entry point.
 *
...
export const server: Plugin = async ({ worktree, client }, options) => {
  ...
  return {
    tool: {
      ...
      triage: tool({
         description:
           "Discover and route to the right specialized skill or agent. " +
           "ALWAYS call this FIRST before attempting any task — check if a specialized skill or agent exists. " +
           ...
           "If an agent matches, route to that agent via the task mechanism. " +
        ...
        async execute(args, context) {
          ...
          if (!query) {
            return "Describe what you need -- triage will find the best matching skill or agent."
          }
    ...
    "tool.definition": async (input, output) => {
      ...
      if (state !== "on") return
      output.description =
        "This tool is disabled. Use `triage` to discover and load specialized skills or route to agents."
