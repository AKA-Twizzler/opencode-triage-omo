#!/usr/bin/env node
/*
 * opencode-triage-omo CLI
 * ==================
 * Manage the opencode-triage-omo agent + skill router plugin.
 *
...
const PLUGIN_NAME = "opencode-triage-omo"
...
function setPluginMode(plugin, mode) {
  ...
  if (mode === "auto") {
    const entry = ["opencode-triage-omo", { autoHide: true }]
    if (idx >= 0) plugin[idx] = entry
    else plugin.push(entry)
  } else {
    if (idx >= 0) {
      plugin.splice(idx, 1)
      plugin.push("opencode-triage-omo")
    } else plugin.push("opencode-triage-omo")
  }
}
...
function showVersion() {
  ...
  console.log(`opencode-triage-omo v${CURRENT_VERSION}`)
}
...
function checkForUpdate() {
  https.get("https://registry.npmjs.org/opencode-triage-omo/latest", { timeout: 3000 }, (res) => {
    let data = ""
    ...
    res.on("end", () => {
      try {
        ...
        if (latest && semverGt(latest, CURRENT_VERSION)) {
          ...
          console.log(YELLOW + `  npm install -g opencode-triage-omo@latest` + RESET)
          console.log()
        }
      } catch {}
    })
...
function showHelp() {
  ...
  console.log()
  console.log(BOLD + "opencode-triage-omo v" + CURRENT_VERSION + RESET + " — Deterministic Skill Router")
  console.log()
  ...
  console.log()
  console.log("  " + BOLD + "Uninstall:" + RESET + " ".repeat(flagCol - "Uninstall:".length) + "npm uninstall -g opencode-triage-omo")
  console.log("  " + BOLD + "Docs:" + RESET + " ".repeat(flagCol - "Docs:".length) + "https://github.com/AKA-Twizzler/opencode-triage-omo")
  console.log()
}
